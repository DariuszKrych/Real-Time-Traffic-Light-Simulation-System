...
def change_states_of_carsW(current_time, street_light_states, _memory={"is_moving": False, "elapsed_time": 0.0, "last_time": None, "chosen_paths": None}):
    speed = 300 
    start_x = 250 
    turn_start_x = 0 
    stop_line_x = 200 
    ...
    if _memory["chosen_paths"] is None:
        _memory["chosen_paths"] = [path_randomiser_W(), path_randomiser_W()]
    ...
    yellow_path_function = _memory["chosen_paths"][0]
    green_path_function = _memory["chosen_paths"][1]

    # Lead car
    yellow_x, yellow_y, yellow_rotation = yellow_path_function(drive_time, speed, start_x, turn_start_x)

    # Trailing car
    time_gap = 0.7 
    green_drive_time = drive_time - time_gap
    green_x, green_y, green_rotation = green_path_function(green_drive_time, speed, start_x, turn_start_x)

    # --- 5. CREATE CARS ---
    yellow_car = ['car_body_1', yellow_x, yellow_y, yellow_rotation, [200, 200, 0, 255]]
    green_car = ['car_body_1', green_x, green_y, green_rotation, [0, 200, 0, 255]]

    return [yellow_car, green_car]